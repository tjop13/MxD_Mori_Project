...
try:
    ...
    if form.getfirst('send') and form.has_key('send'):
        ...
        cur = con.cursor()
        try:
            cur.execute("SELECT count(*) FROM boardtbl WHERE LocationID='1' ")
            IDnum = cur.fetchone()[0]
            cur.execute("SELECT * FROM boardtbl WHERE ID=:id AND name=:Name AND comment=:Comment",{"id":IDnum,"Name":name,"Comment":comment})
            if cur.fetchone() == None:
                IDnum += 1
                cur.execute("INSERT INTO boardtbl(ID,LocationID,Genre,regdate,name,comment,Replyname,Replycomment,SolvedFlag) values(?,'1',?,datetime('now','localtime'),?,?,'','','0')",(IDnum,cgi.escape(genre),name,comment))
            con.commit()
        except:
            con.rollback()
        finally:
