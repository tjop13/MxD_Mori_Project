...
try:
    ...
    cur.executescript("""CREATE TABLE Instractiontbl(UNIQUEID integer primary key autoincrement, ID integer, LocationID varchar(5), regdate timestamp, name varchar(100),comment varchar(1024),Replyname varchar(100),Replycomment varchar(1024),SolvedFlag varchar(1), Send varchar(2));""")
    ...
    try:
        ...
        for each in cur.fetchall():
            ...
            cur.execute("UPDATE Patienttbl SET Send='1' WHERE ID=:id",{"id":each['ID']})
            con.commit()
        print "$#&"
        ...
        for each in cur.fetchall():
            ...
            cur.execute("UPDATE Chronologytbl SET Send='1' WHERE ID=:id",{"id":each['ID']})
            con.commit()
        print "$#&"
        ...
        for each in cur.fetchall():
            ...
            cur.execute("UPDATE Instractiontbl SET Send='1' WHERE ID=:id",{"id":each['ID']})
            con.commit()
        print "$#&"

        cur.execute("SELECT DISTINCT * FROM Victortbl ORDER BY regdate DESC")
        for each in cur.fetchall():
            ...
            cur.execute("UPDATE Victortbl SET Send='1' WHERE ID=:id",{"id":each['ID']})
            con.commit()
