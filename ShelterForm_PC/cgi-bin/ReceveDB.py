...
try:
    ...
    cur.executescript("""CREATE TABLE LogData(UNIQUEID integer primary key autoincrement, regdate timestamp, class varchar(10), ID integer);""")
    cur.close()
except:
    print
finally:
    ...
    if  form.has_key("ID") and form.has_key('LocationID') and form.has_key("Class"):
        ...
        cur.execute("INSERT INTO LogData(regdate,class,ID,LocationID) values(datetime('now','localtime'),?,?,?)",(form["Class"].value,ID,LocationID))
